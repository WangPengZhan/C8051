...
uint8_t I2C_Master_start(void)
{
	...
	if (( Get_SCL() == 1 ) && ( Get_SDA() == 1 ))
	{
		...
		delay_us_timer1(I2C_Master_HoldTime);
		SCL_L();
		delay_us_timer1(I2C_Master_Delay);  // 为了保持SCL为低电平的时间约为半周期
...
uint8_t I2C_Master_stop(void)
{
	...
	delay_us_timer1(Rise_Time_Delay); // 等待SCL SDA稳定
...
uint8_t I2C_Master_bit_out( uint8_t bit_out)
{
	...
	delay_us_timer1(Rise_Time_Delay);              // 要求等待时间大于2us
	...
	delay_us_timer1( I2C_Master_Delay );            // 延时，控制传送速率
...
uint8_t I2C_Master_bit_in( uint8_t *bit_in)
{
	...
	delay_us_timer1(Rise_Time_Delay); 	// 等待SDA线路稳定

	SCL_HIZ();                      	// 拉高时钟线

	timercount = SCL_Stretch_Timeout;   // 设置超时等待时间，时钟周期：8

	delay_us_timer1(Rise_Time_Delay);   // 等待SCL线路稳定
...
uint8_t I2C_Master_Init(void)
{
	...
	delay_us_timer1(Rise_Time_Delay);            // 等待时钟上升沿完成
